...
public class RUMDbSearchEngine {
	...
	public void insertMoviesIntoHashTable ( String inputFile ) {
        ...
        for(ArrayList<String> first: allMovies)
        {
            String title = first.get(0); 

            for(int i = 1; i<first.size(); i++)
            {
                String word = isWord(first.get(i)); 
                if(word != null)
                {
	...
	public void insertWordLocation (String word, Location loc) {
        ...
        WordOccurrence wo = getWordOccurrence(word);
        if(wo != null)
        {
            wo.addOccurrence(loc);
            return; 
        }
        
        wo = new WordOccurrence(word); 
        wo.addOccurrence(loc);
        WordOccurrence head = hashTable[hashFunction(word)]; 
        wo.next = head; 
        hashTable[hashFunction(word)] = wo; 
        wordCount++; 
    ...
    private void rehash (int newHashSize){
        ...
        //StdOut.println("rehash called"); 
        WordOccurrence[] oldht = hashTable; 
        ...
        {
            WordOccurrence ptr = oldht[i]; 
            while(ptr!=null)
            {
                WordOccurrence ptrnext = ptr.next; 
                oldht[i] = ptrnext; 
                String s = ptr.getWord(); 
                //ArrayList<Location> l = ptr.getLocations(); 
                int newIndex = hashFunction(s); 
                insert(ptr,newIndex); 
                ptr = oldht[i]; 
            }
        }
    ...
    private void insert(WordOccurrence wo, int newIndex)
    {
        WordOccurrence head = hashTable[newIndex]; 
        wo.next = head; 
        hashTable[newIndex] = wo;  
        wordCount++; 
        if(getLoadFactor() > threshold)
        {
            rehash(2*hashSize);
	...
	public WordOccurrence getWordOccurrence (String word) {
        ...
        for(WordOccurrence wo: hashTable)
        {
            WordOccurrence ptr = wo; 
            while(ptr!= null)
            {
	...
	public ArrayList<MovieSearchResult> createMovieSearchResult (String wordA, String wordB) {
        ...
        // COMPLETE THIS METHOD
        ArrayList<MovieSearchResult> f = new ArrayList<MovieSearchResult>(); 
        WordOccurrence woa = getWordOccurrence(wordA); 
        WordOccurrence wob = getWordOccurrence(wordB);

        if(woa== null || wob == null)
        {
            return null; 
        }

        ArrayList<Location> locations = woa.getLocations(); 
        boolean mSAExists = false; 
        for(Location l: locations)
        {
            for(MovieSearchResult m: f)
            {
                if(l.getTitle().equals(m.getTitle()))
                {
                    m.addOccurrenceA(l.getPosition());
                    mSAExists = true; 
                    break; 
                }
            }
            ...
            if(mSAExists == false)
            {
                MovieSearchResult msr = new MovieSearchResult(l.getTitle());
                msr.addOccurrenceA(l.getPosition());
                f.add(msr);
            }
            mSAExists = false; 
        }
        
        ArrayList<Location> locationss = wob.getLocations();
        boolean mSExists = false; 
        for(Location l: locationss)
        {
            for(MovieSearchResult m: f)
            {
                if(l.getTitle().equals(m.getTitle()))
                {
                    m.addOccurrenceB(l.getPosition());
                    mSExists = true; 
                    break; 
                }
            }
            ...
            {
                MovieSearchResult msr = new MovieSearchResult(l.getTitle());
                msr.addOccurrenceB(l.getPosition());
                f.add(msr);
            }
            mSExists = false; 
        }
	...
	public ArrayList<MovieSearchResult> topTenSearch(String wordA, String wordB){
        ...
        // COMPLETE THIS METHOD
        ArrayList<MovieSearchResult> f = new ArrayList<MovieSearchResult>(); 
        ArrayList<MovieSearchResult> ff = createMovieSearchResult(wordA, wordB);
        ...
        for(MovieSearchResult ms: ff)
        {
            calculateMinDistance(ms);
        }
