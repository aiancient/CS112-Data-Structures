...
public class Percolation {
	...
	*/
	...
	{
		int spotInGrid = gridSize*row + col;
		if(col >= 1 && grid[row][col-1]) //left
		{
			wquFind.union(spotInGrid, spotInGrid-1);
		}

		if(col <= gridSize - 2 && grid[row][col+1]) //right
		{
			wquFind.union(spotInGrid, spotInGrid+1);
		}

		if(row >= 1 && grid[row-1][col]) //top
		{
			wquFind.union(spotInGrid, spotInGrid-gridSize);
		}

		if(row <= gridSize - 2&& grid[row+1][col]) //bottom
		{
			wquFind.union(spotInGrid, spotInGrid+gridSize);
		}
	}
